...
public sealed class NetworkManager : IDisposable
{
    ...
    /// <summary>
    /// A direct connection to this node dropped.  Distinct from PeerRemoved,
    /// which only fires for relayed peers going away: a direct peer is kept in
    /// the list and merely marked offline, so nothing else signalled "any work
    /// in flight with this node is now dead".  TransferManager needs that to
    /// release half-written files.
    /// </summary>
    public event Action<string>? PeerDisconnected;   // node-id
    ...
    /// <param name="exclusivePort">
    /// True (desktop): claim the port exclusively so a second copy fails to
    /// bind — this is a single-instance app and the caller turns an in-use
    /// failure into "activate the running copy".  False (mobile): Android only
    /// ever runs one instance anyway, and SO_REUSEADDR avoids a bind failure
    /// when the app is restarted while the old socket is still in TIME_WAIT,
    /// which happens constantly during development.
    /// </param>
    public NetworkManager(ILanLinkSettings settings,
                          IPlatformSupport? platform = null,
                          bool exclusivePort = true)
    {
        _settings      = settings;
        _exclusivePort = exclusivePort;
        _discovery = new DiscoveryService(settings.NodeId, settings.DisplayName,
                                          settings.Port, platform);
    ...
    public void Start()
    {
        try
        {
            ...
            // See the exclusivePort ctor parameter for why this differs by platform.
            if (_exclusivePort)
            {
        ...
        catch (Exception ex)
        {
            throw new InvalidOperationException(
                $"TCP listener on port {_settings.Port}: {ex.GetType().Name}: {ex.Message}", ex);
        }
        ...
        {
            throw new InvalidOperationException(
                $"UDP discovery on port {_settings.Port}: {ex.GetType().Name}: {ex.Message}", ex);
        }
    ...
    private void OnConnectionDisconnected(PeerConnection conn)
    {
        ...
        // Tell anyone holding state for this node that it is over — notably
        // TransferManager, which otherwise keeps half-written files open forever.
        PeerDisconnected?.Invoke(conn.RemoteNodeId);
