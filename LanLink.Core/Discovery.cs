...
public sealed class DiscoveryService : IDisposable
{
    ...
    private readonly IPlatformSupport? _platform;
    private IDisposable? _multicastLock;
    ...
    public DiscoveryService(string nodeId, string name, int port, IPlatformSupport? platform = null)
    {
        _nodeId = nodeId;
        _name   = name;
        _port   = port;
    ...
    public void Start()
    {
        ...
        // Android filters broadcast/multicast away from user space unless a
        // MulticastLock is held, so without this the listener receives nothing.
        // Desktop supplies no platform hook and this is simply null.
        _multicastLock = _platform?.AcquireMulticastLock("LanLink.Discovery");

        // AddressFamily.InterNetwork + SO_BROADCAST explicitly: the mobile copy
        // needed both, and stating them costs the desktop nothing.
    ...
    public void Dispose()
    {
        ...
        // Releasing the multicast lock matters on Android: held locks keep the
        // WiFi chip delivering broadcast traffic and cost battery.
        try { _multicastLock?.Dispose(); } catch { }
        _multicastLock = null;
    }
}
