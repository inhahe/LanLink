...
public sealed class WireMessage
{
    ...
    /// <summary>
    /// dir_start only: every sub-directory's relative path, so the receiver can
    /// recreate the tree faithfully.  Without it, directories only ever appear
    /// as a side effect of writing a file into them, and an **empty** folder is
    /// silently dropped — the sender reports success and the folder just isn't
    /// there.  Nullable and additive, so an older peer that doesn't know the
    /// field simply ignores it and behaves as before.
    /// </summary>
