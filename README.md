...
- **Port 48656** — TCP for data, UDP for discovery (same port number, different protocols)
...
On first launch (Windows), the app creates inbound firewall rules for TCP/UDP port 48656 via an elevated `netsh` command (you'll see one UAC prompt). If you decline, LAN discovery won't work until you manually allow the port.
...
1. Forward port 48656 (TCP) on the remote router to the target machine
